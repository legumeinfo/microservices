...
@routes.get("/files/{prefix}")
async def files_for_prefix(request: web.Request) -> web.Response:
    """Return canonical protein/CDS/genome FASTA URLs for an annotation prefix.

    :param request: The incoming aiohttp request. The ``prefix`` path segment
        is a full-yuck annotation prefix (e.g. ``glyma.Wm82.gnm2.ann1``).
    :return: A JSON response with the file URLs, or 404 if no annotation
        matches the prefix.
    """
    handler = request.app["handler"]
