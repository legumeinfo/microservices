...
def transferChromosomes(postgres_connection, redisearch_loader, uniquename):
  ...
  with postgres_connection.cursor() as c:
    ...
    for (pk, name, organism_id, length,) in c:
      chromosome_id_name_map[pk] = name
      organism = organism_id_map[organism_id]
      redisearch_loader\
        .indexChromosome(name, length, organism['genus'], organism['species'])

    return chromosome_id_name_map


def transferGenes(postgres_connection, redisearch_loader,
...
uniquename):
  ...
  postgres_connection_string = \
    makePostgresConnectionString(database, user, password, host, port)
