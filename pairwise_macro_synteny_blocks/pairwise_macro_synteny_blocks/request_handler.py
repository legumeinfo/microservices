...
class RequestHandler:
    ...
    def _indexPairsToIndexBlocks(self, pairs, intermediate, matched):
        ...
        f = self._indexBlocksViaIndexPathTraceback(
            f_path_ends, f_pointers, f_scores, matched
        )
        r = self._indexBlocksViaIndexPathTraceback(
            r_path_ends, r_pointers, r_scores, matched
        )
        return chain(f, r)
    ...
    async def process(
        ...
        target_chromosome = await self.redis_connection.lrange(
            f"{target_doc_id}:families", 0, -1
        )

        # compute gene index pairs based on matching annotations
        index_pairs, masked_families = self._chromosomesToIndexPairs(
            query_chromosome, target_chromosome, mask
        )
