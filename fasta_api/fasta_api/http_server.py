...
def run_http_server(host, port, handler):
    ...
    cors.add(route)
    route = app.router.add_get("/fasta/fetch/{seqid}:{start}-{end}/{url:path}", http_fasta_range)
    cors.add(route)
    route = app.router.add_get("/fasta/fetch/{seqid}/{url:path}", http_fasta_range)
    cors.add(route)
    route = app.router.add_get("/fasta/references/{url:path}", http_fasta_references)
    cors.add(route)
    route = app.router.add_get("/fasta/lengths/{url:path}", http_fasta_lengths)
    cors.add(route)
    route = app.router.add_get("/fasta/nreferences/{url:path}", http_fasta_nreferences)
    cors.add(route)
    route = app.router.add_get("/gff/contigs/{url:path}", http_gff_references)
    cors.add(route)
    route = app.router.add_get("/gff/fetch/{seqid}:{start}-{end}/{url:path}", http_gff_features)
    cors.add(route)
    route = app.router.add_get("/gff/fetch/{seqid}/{url:path}", http_gff_features)
    cors.add(route)
    route = app.router.add_get("/bed/fetch/{seqid}:{start}-{end}/{url:path}", http_bed_features)
    cors.add(route)
    route = app.router.add_get("/bed/fetch/{seqid}/{url:path}", http_bed_features)
    cors.add(route)
    route = app.router.add_get("/vcf/contigs/{url:path}", http_vcf_contigs)
    cors.add(route)
    route = app.router.add_get("/vcf/fetch/{seqid}:{start}-{end}/{url:path}", http_vcf_features)
    cors.add(route)
    route = app.router.add_get("/vcf/fetch/{seqid}/{url:path}", http_vcf_features)
    cors.add(route)
    route = app.router.add_get("/alignment/references/{url:path}", http_alignment_references)
    cors.add(route)
    route = app.router.add_get("/alignment/unmapped/{url:path}", http_alignment_unmapped)
    cors.add(route)
    route = app.router.add_get("/alignment/nreferences/{url:path}", http_alignment_nreferences)
    cors.add(route)
    route = app.router.add_get("/alignment/nocoordinate/{url:path}", http_alignment_nocoordinate)
    cors.add(route)
    route = app.router.add_get("/alignment/mapped/{url:path}", http_alignment_mapped)
    cors.add(route)
    route = app.router.add_get("/alignment/lengths/{url:path}", http_alignment_lengths)
    cors.add(route)
    route = app.router.add_get("/alignment/index_statistics/{url:path}", http_alignment_index_statistics)
    cors.add(route)
    route = app.router.add_get("/alignment/count/{contig}:{start}-{stop}/{url:path}", http_alignment_count)
    cors.add(route)
    route = app.router.add_get("/alignment/count_coverage/{contig}:{start}-{stop}/{url:path}", http_alignment_count_coverage)
    cors.add(route)
    route = app.router.add_get("/alignment/fetch/{contig}:{start}-{stop}/{url:path}", http_alignment_fetch)
    cors.add(route)
    route = app.router.add_get("/alignment/fetch/{contig}/{url:path}", http_alignment_fetch)
    cors.add(route)
    route = app.router.add_get("/alignment/length/{reference}/{url:path}", http_alignment_reference_lengths)
    cors.add(route)
    # run the app
    web.run_app(app)
