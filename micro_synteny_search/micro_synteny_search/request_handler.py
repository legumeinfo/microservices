...
class RequestHandler:
    ...
    async def _chromosomeBlocksToTracks(
        self, chromosome_name, blocks, chromosome_index
    ):
        chromosome_doc = await chromosome_index.load_document(
            f"chromosome:{chromosome_name}"
        )
        tracks = await asyncio.gather(
            *[self._blockToTrack(chromosome_doc, block) for block in blocks]
        )
        return tracks
    ...
    async def _queryToChromosomeGeneMatchIndexes(self, query_track, gene_index):
        ...
        for family in families:
            ...
            query = Query(query_string).paging(0, 0)
    ...
    async def _chromosomeGeneIndexesToBlocks(
        self, query_track, chromosome_match_indices, matched, intermediate
    ):
        ...
        for chromosome_name, indices in chromosome_match_indices.items():
            ...
            for i in indices[1:]:
                ...
                if (
                    intermediate < 1
                    and (i - block[-1]) / len(query_track) <= intermediate
    ...
    async def process(self, query_track, matched, intermediate):
        ...
        chromosome_match_indexes = await self._queryToChromosomeGeneMatchIndexes(
            query_track, gene_index
        )
        # compute micro-synteny blocks
        blocks = await self._chromosomeGeneIndexesToBlocks(
            query_track, chromosome_match_indexes, matched, intermediate
        )
        # fetch result tracks
