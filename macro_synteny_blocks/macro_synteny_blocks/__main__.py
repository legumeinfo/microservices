...
from macro_synteny_blocks.database import connectToRedis
from macro_synteny_blocks.grpc_server import run_grpc_server
...
def parseArgs():
  ...
  parser = argparse.ArgumentParser(
    prog=macro_synteny_blocks.__name__,
    description='A microservice for finding chromosome names similar to the given query.',
    formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  parser.add_argument(
    '--version',
    action='version',
    version=f'%(prog)s {macro_synteny_blocks.__version__} schema {macro_synteny_blocks.__schema_version__}',
  )
  ...
  parser.add_argument('--pairwiseaddr', action=EnvArg, envvar=pairwiseaddr_envvar, type=str, required=True, help=f'The address of the pairwise macro-synteny microservice (can also be specified using the {pairwiseaddr_envvar} environment variable).')
