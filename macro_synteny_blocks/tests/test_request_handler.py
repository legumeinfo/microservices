...
class TestGetTargets:
    ...
    async def test_batching_with_large_family_set(self, redis_connection):
        ...
        for i in range(10):
            await redis_connection.hset(f"gene:test_batch_gene_chr1_{i}", mapping={
        ...
        for i in range(5):
            await redis_connection.hset(f"gene:test_batch_gene_chr2_{i}", mapping={
    ...
    async def test_filtering_by_matched(self, redis_connection):
        ...
        chromosome = ["test_match_fam1", "test_match_fam2", "test_match_fam3", "test_match_fam4"]

        # chr1 has only 1 match (below matched=3)
        await redis_connection.hset("gene:test_match_chr1_gene1", mapping={
        ...
        for i in range(4):
            await redis_connection.hset(f"gene:test_match_chr2_gene{i}", mapping={
    ...
    async def test_filtering_by_intermediate(self, redis_connection):
        ...
        chromosome = ["test_inter_fam1", "test_inter_fam2", "test_inter_fam3", "test_inter_fam4"]

        # chr1: matches at indices 0, 1, 2 (close together)
        for i in range(3):
            await redis_connection.hset(f"gene:test_inter_chr1_gene{i}", mapping={
        ...
        for idx, i in enumerate([0, 10, 20]):
            await redis_connection.hset(f"gene:test_inter_chr2_gene{idx}", mapping={
    ...
    async def test_with_targets_filter(self, redis_connection):
        ...
        for i in range(3):
            await redis_connection.hset(f"gene:test_target_chr1_gene{i}", mapping={
                "name": f"test_target_chr1_gene{i}",
                "family": f"test_target_fam{i + 1}",
                "chromosome": "test_target_chr1",
                "index": str(i),
            })
            await redis_connection.hset(f"gene:test_target_chr2_gene{i}", mapping={
...
class TestEnrichBlocksWithGeneInfo:
    ...
    async def test_enrichment_with_genes_address(self, sample_blocks, sample_query_gene_names, mock_genes_service):
        ...
        with patch("macro_synteny_blocks.request_handler.getGenes", new=mock_genes_service):
            enriched = await self.handler._enrichBlocksWithGeneInfo(
                sample_blocks,
                sample_query_gene_names
            )
    ...
    async def test_enrichment_uses_both_terminal_genes(self, sample_blocks, sample_query_gene_names, mock_genes_service):
        ...
        with patch("macro_synteny_blocks.request_handler.getGenes", new=tracking_mock_getGenes):
    ...
    async def test_enrichment_without_genes_address(self, sample_blocks, sample_query_gene_names):
    ...
    async def test_enrichment_handles_grpc_objects(self, sample_query_gene_names, mock_genes_service):
    ...
    async def test_enrichment_with_min_max_calculation(self, sample_query_gene_names, mock_genes_service):
...
class TestAddChromosomeLengths:
    ...
    async def test_add_chromosome_lengths(self, redis_with_chromosomes, sample_blocks):
        ...
        handler = RequestHandler(
            redis_connection=redis_with_chromosomes,
            pairwise_address="localhost:8081"
        )
...
class TestProcessWithChromosomeName:
    ...
    async def test_process_with_chromosome_name_success(self, fakeredis_connection, mock_chromosome_service, mock_genes_service, mock_pairwise_service):
        ...
        await fakeredis_connection.hset("chromosome:chr1", mapping={
            "name": "chr1",
            "genus": "Test",
            "species": "species",
            "length": "10000",
        ...
        with patch("macro_synteny_blocks.request_handler.getChromosome", new=mock_chromosome_service):
            with patch("macro_synteny_blocks.request_handler.getGenes", new=mock_genes_service):
                with patch("macro_synteny_blocks.request_handler.computePairwiseMacroSyntenyBlocks", new=mock_pairwise_service):
    ...
    async def test_process_with_chromosome_name_not_found(self, fakeredis_connection, mock_chromosome_service):
        ...
        with patch("macro_synteny_blocks.request_handler.getChromosome", new=mock_getChromosome_none):
...
class TestGrpcBlockToDictBlock:
    ...
    def test_basic_conversion(self):
        ...
        Block = namedtuple("Block", ["i", "j", "fmin", "fmax", "orientation", "optionalMetrics"])
    ...
    def test_conversion_with_metrics(self):
        ...
        Block = namedtuple("Block", ["i", "j", "fmin", "fmax", "orientation", "optionalMetrics"])
        grpc_block = Block(
            i=0, j=5, fmin=0, fmax=5000, orientation="+",
            optionalMetrics=[0.5, 0.8]
        )
